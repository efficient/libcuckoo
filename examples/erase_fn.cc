...
typedef cuckoohash_map<std::string, std::shared_ptr<InnerTable>> OuterTable;
...
int main() {
	...
	tbl.insert("bob",  std::shared_ptr<InnerTable>(new InnerTable));
	tbl.update_fn("bob", []( std::shared_ptr<InnerTable>& innerTbl) {
		innerTbl->insert("nickname", "jimmy");
		innerTbl->insert("pet", "dog");
		innerTbl->insert("food", "bagels");
	});

	tbl.insert("jack",  std::shared_ptr<InnerTable>(new InnerTable));
	tbl.update_fn("jack", []( std::shared_ptr<InnerTable>& innerTbl) {
	...
	tbl.erase_fn("jack", []( std::shared_ptr<InnerTable>& innerTbl) {
		std::string out;
