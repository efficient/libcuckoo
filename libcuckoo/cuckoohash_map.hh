...
private:
     ...
     * for a table and returns the smallest hashpower that will hold n
     * elements. */
    ...
    static bool cuckoopath_move(
        ...
        while (depth > 0) {
            ...
            if (depth == 1) {
                 ...
                 * since at the end of this function, i1 and i2 must be
                 * locked. */
                ob = (fb == i1) ? i2 : i1;
...
public:
     ...
     * For the duration of its existence, it takes all the locks on the table it
     * is given, thereby ensuring that no other threads can modify the table
     ...
     * \ref snapshot_table method if possible, since it is less error-prone. The
     * iterator allows movement forward and backward through the table as well
     * as dereferencing items in the table. It maintains the invariant that the
     * iterator is either an end iterator (which points past the end of the
     * table), or points to a filled slot. As soon as the iterator looses its
     * lock on the table, all dereference and movement operations will throw an
     * exception. */
    ...
    public:
        ...
        /*! This is an rvalue-reference constructor that takes the lock from \p
          it and copies its state. To create an iterator from scratch, call the
          \ref cbegin or \ref cend methods of cuckoohash_map. */
    ...
    public:
         ...
         *  std::pair which contains const references to the key and value under
         *  the iterator. */
        ref_pair* operator->() {
            check_lock();
            if (is_end()) {
         ...
         * next nonempty slot. If it reaches the end of the table, it becomes an
         * end iterator. It throws an exception if the iterator is already at
    ...
    public:
         ...
         * involves modifying the hash table itself, but since we have a lock on
         * the table, we are okay. We are only changing the value in the bucket,
         * so the element will retain it's position in the table. */
